...
puzzle_input = '''muted white bags contain 4 dark orange bags, 3 bright white bags.
...
def find_gold(tree, containers=None):
    global count
    if containers is None:
        containers = ["shinygold"]


    for bag, contents in tree.items():
                
             
        for c in containers:
            if c in contents and bag not in containers:
                containers.append(bag)
                count += 1
                find_gold(tree, containers)
            
            


bagcount = 0
def find_bags_inside(tree, bag="shinygold"):
    global bagcount

   
    bagcount += len(tree[bag])

    for subbag in tree[bag]:       

        
        find_bags_inside(tree, subbag)
...
def solve(data):
    ...
    for line in data:
        ...
        bag, components = line.split(" contain ")
        ...
        if components[0].split(" ")[0] == "no":
            components = []
        else:
            for c in components:
                bag_type = "".join(c.strip().split(" ")[1:-1]        )
                bag_num = int(c.strip().split(" ")[0])
                new_comp += [bag_type] * bag_num
