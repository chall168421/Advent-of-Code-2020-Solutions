...
rules = """byr (Birth Year) - four digits; at least 1920 and at most 2002.
...
for passport in data:
     ...
     byr = int(passport.split("byr:")[-1].split()[0].strip())
     iyr = int(passport.split("iyr:")[-1].split()[0].strip())
     eyr = int(passport.split("eyr:")[-1].split()[0].strip())
     hgt = passport.split("hgt:")[-1].split()[0].strip()
     hcl = passport.split("hcl:")[-1].split()[0].strip()
     ecl = passport.split("ecl:")[-1].split()[0].strip()
     pid = passport.split("pid:")[-1].split()[0].strip()
     ...
     try:
          ...
          if hgt.endswith("cm"):
               hgt = int(hgt.replace("cm", "").strip())
               if hgt < 150 or hgt > 193:
                    valid = False
               
          elif hgt.endswith("in"):
               hgt = int(hgt.replace("in", "").strip())
               if hgt < 59 or hgt > 76:
                    valid = False
          ...
          if search("#[0-9a-f]{6}", hcl) is None or len(hcl) != 7:
               valid = False

          
          if not any([ecl==c for c in "amb blu brn gry grn hzl oth".split(" ")]):
               valid = False
